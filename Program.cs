...
namespace Battleships
{
    class Program
    {
        ...
        static void SetComputerGrid()
        {
            ...
            bool gen = false;
                do
                {
                    ...
                    // generate random values for each x and y ordinates
                    int GenPosX = rnd.Next(0,8);
                    int GenPosY = rnd.Next(0,8);
                    // prevention of repeat coordinates
        ...
        static void ReadAndLoadFile()
         {
            ...
            for (int y = 0; y < 8; y++)
            {
                // creates a tempory array of all the characters split from one line of the text file, adds to corresponding array
                string[] row = (File.ReadLines(filename).Skip(3+y).Take(1).First()).Split(",");
                for (int x = 0; x < 8; x++)
                {
                    // sets corresponding array equal to value stored at coordinate in text file
        ...
        {
            // init bool variable for error prevention, loop cannot be broken unless code is told to break
            bool deploy = false;
            do
            {
                // ask user for the coordinates they would like to target
                PrintWithRed("<OUTPUT> ENTER TARGET COORDINATES : FORMAT [A,1]\n");
                System.Console.Write("<INPUT> ");
                // string to store coord input from user
                string targetCoords = Console.ReadLine().ToUpper();
                // removes any erroneous characters that could be input by mistake
                targetCoords = targetCoords.Trim(charsToTrim);
                // array to store substrings after delimit
                string[] attackCoords;
                // split into separate characters after every comma
                attackCoords = targetCoords.Split(',');
                // if coordinate is in target format, then execute code in if block
                ...
                // error handling if format is incorrect or invalid.
                else
                {
                    // error message for handling
                    PrintWithYellow("\nCoordinates invalid, retry.\n");
        ...
        {
            // random local variable for generating pseudorandom positions on the grid
            Random rnd = new Random();
            // integer variable for the x-ordinate
            int PosX = rnd.Next(0,8);
            // integer variable for the y-ordinate
            int PosY = rnd.Next(0,8);
            // calls the function to identify if a target was at the position
            CheckHitOrMiss(PosX, PosY, turn);
            // display user with the generated coordinates that was attacked
        ...
        static void PlayGame()
        {
            ...
            do {
                ...
                SaveFile(turn);
                // while loop breakes once one player reaches 5 target strikes
            } while (PlayerStrikes < 5 && ComputerStrikes < 5);

            // identify which message to output based on which side won
            if (PlayerStrikes == 5)
            {
                PrintWithBlue("Player Wins!");
                StreamWriter strm = File.CreateText(@"GameData.txt");
                strm.Flush();
                strm.Close();
            }
        ...
        static void ResumeFromSave()
        {
            ...
            // loads data stored in txt file into neccessary arrays and variables
            ReadAndLoadFile();
